...
# --- REUSABLE SIMULATION ENGINE (The "Quant Engine") ---
def run_endowment_sim(v0, years, spend_rate, r, vol, inf, fee, sims, use_tails, rule, fund_percentile, cap_shock_amt=0, cap_shock_yr=0):
    ...
    # Track spending for the Hybrid Rule (Yale Model)
    # Initial daily spend for smoothing
    prev_spend = np.full(sims, (v0 * spend_rate) / 252) 
    
    for i in range(1, steps):
        # Stochastic Shock: Student-t (Fat Tails) vs Normal
        if use_tails:
        ...
        # Determine Spending Rule Logic
        if rule == "Yale Hybrid":
            # 80% Weight on Inflation-Adjusted Prior Spend + 20% on Market Value
            current_spend = (0.8 * prev_spend * (1 + inf*dt)) + (0.2 * (paths[:, i-1] * spend_rate / 252))
        else:
            # Standard Fixed Percentage (Inflation Adjusted)
            current_year = i // 252
            inf_adj = (1 + inf) ** current_year
            current_spend = (v0 * spend_rate * inf_adj) / 252
            
        # Apply Capital Project Shock (Lump sum withdrawal at specific year)
        shock_term = 0
        ...
        prev_spend = current_spend # Update for next daily step
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        if 'returns' in df.columns:
            ...
            volatility = hist_vol # Overwrite with real data

# --- MAIN DASHBOARD: CORE ANALYSIS ---
...
# --- SECTION 3: STRATEGIC SENSITIVITY ---
st.markdown("---")
...
if st.button("Generate Heatmap Analysis"):
    with st.spinner("Analyzing 25,000 potential market regimes..."):
        # Setup sensitivity grid
        rates = np.linspace(0.03, 0.06, 5) # Spending rates 3% to 6%
        vols = np.linspace(0.10, 0.25, 5)  # Volatilities 10% to 25%
        heatmap_data = np.zeros((5, 5))

        for r_idx, r in enumerate(rates):
            for v_idx, v in enumerate(vols):
                # Run mini-sim (500 iterations for speed)
